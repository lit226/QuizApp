...
class GKDifficultyLavelViewController: UIViewController {
    ...
    func setup() {
        ...
        difficultyView.easyCellTapped = { [weak self] () -> Void in
            ...
            APICaller.shared.fetchGKEasyQuestion { results in
                switch results {
        ...
        difficultyView.mediumCellTapped = { [weak self] () -> Void in
            let questionsVC = QuestionViewController()
            APICaller.shared.fetchGKMediumQuestion { results in
                switch results {
                case .success(let result):
                    questionsVC.configure(result: result)
                case .failure(let error):
        ...
        difficultyView.hardCellTapped = { [weak self] () -> Void in
            let questionsVC = QuestionViewController()
            APICaller.shared.fetchGKHardQuestion { results in
                switch results {
                case .success(let result):
                    questionsVC.configure(result: result)
                case .failure(let error):
