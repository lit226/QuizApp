...
class SportsDiffucultyLevelViewController: UIViewController {
    ...
    func setup() {
        ...
        difficultyView.easyCellTapped = { [weak self] () -> Void in
            ...
            APICaller.shared.fetchSportsEasyQuestion { results in
                switch results {
        ...
        difficultyView.mediumCellTapped = { [weak self] () -> Void in
            let questionsVC = QuestionViewController()
            APICaller.shared.fetchSportsMediumQuestion { results in
                switch results {
                case .success(let result):
                    questionsVC.configure(result: result)
                case .failure(let error):
        ...
        difficultyView.hardCellTapped = { [weak self] () -> Void in
            let questionsVC = QuestionViewController()
            APICaller.shared.fetchSportsHardQuestion { results in
                switch results {
                case .success(let result):
                    questionsVC.configure(result: result)
                case .failure(let error):
