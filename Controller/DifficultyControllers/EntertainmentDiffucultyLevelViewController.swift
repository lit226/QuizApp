...
class EntertainmentDiffucultyLevelViewController: UIViewController {
    ...
    func setup() {
        ...
        difficultyView.easyCellTapped = { [weak self] () -> Void in
            ...
            APICaller.shared.fetchEntertainmenteEasyQuestion { results in
                switch results {
        ...
        difficultyView.mediumCellTapped = { [weak self] () -> Void in
            let questionsVC = QuestionViewController()
            APICaller.shared.fetchEntertainmenteMediumQuestion { results in
                switch results {
                case .success(let result):
                    questionsVC.configure(result: result)
                case .failure(let error):
        ...
        difficultyView.hardCellTapped = { [weak self] () -> Void in
            let questionsVC = QuestionViewController()
            APICaller.shared.fetchEntertainmenteHardQuestion { results in
                switch results {
                case .success(let result):
                    questionsVC.configure(result: result)
                case .failure(let error):
